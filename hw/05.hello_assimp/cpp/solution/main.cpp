...
void extract_scene_info()
{
  // int num_of_cameras = g_cameras.size();
  // std::cout << g_cameras.size() << std::endl;
  // for (int cam_idx = 0; cam_idx < num_of_cameras; cam_idx++)
  // {
  //   glm::vec3 current_cam_pos = g_cameras[cam_idx].position();
  //   glm::quat current_cam_rotation = g_cameras[cam_idx].get_rotation();
    
  //   std::cout << current_cam_pos.x << " " << current_cam_pos.y << " " << current_cam_pos.z << std::endl;
  //   std::cout << current_cam_rotation.w << " " << current_cam_rotation.x << " " << current_cam_rotation.y << " " << current_cam_rotation.z << std::endl;
  // }

  // int num_of_objects = g_objects.size();
  // for (int obj_idx = 0; obj_idx < num_of_objects; obj_idx++)
  // {
  //   glm::vec3 current_obj_pos = g_objects[obj_idx].get_translate();
  //   glm::quat current_cam_rotation;
  //   g_objects[obj_idx].get_rotate(current_cam_rotation);
  //   glm::vec3 current_obj_scale = g_objects[obj_idx].get_scale();
    
  //   std::cout << current_obj_pos.x << " " << current_obj_pos.y << " " << current_obj_pos.z << std::endl;
  //   std::cout << current_cam_rotation.w << " " << current_cam_rotation.x << " " << current_cam_rotation.y << " " << current_cam_rotation.z << std::endl;
  //   std::cout << current_obj_scale.x << " " << current_obj_scale.y << " " << current_obj_scale.z << std::endl;
  // }

  std::ofstream outputFile("scene_info.txt"); // 출력을 저장할 파일 스트림 생성

    int num_of_cameras = g_cameras.size();
    outputFile << g_cameras.size() << std::endl;
    for (int cam_idx = 0; cam_idx < num_of_cameras; cam_idx++)
    {
        glm::vec3 current_cam_pos = g_cameras[cam_idx].position();
        glm::quat current_cam_rotation = g_cameras[cam_idx].get_rotation();

        outputFile << current_cam_pos.x << " " << current_cam_pos.y << " " << current_cam_pos.z << std::endl;
        outputFile << current_cam_rotation.w << " " << current_cam_rotation.x << " " << current_cam_rotation.y << " " << current_cam_rotation.z << std::endl;
    }

    int num_of_objects = g_objects.size();
    outputFile << num_of_objects << std::endl;
    for (int obj_idx = 0; obj_idx < num_of_objects; obj_idx++)
    {
        glm::vec3 current_obj_pos = g_objects[obj_idx].get_translate();
        glm::quat current_cam_rotation;
        ...
        outputFile << current_obj_pos.x << " " << current_obj_pos.y << " " << current_obj_pos.z << std::endl;
        outputFile << current_cam_rotation.w << " " << current_cam_rotation.x << " " << current_cam_rotation.y << " " << current_cam_rotation.z << std::endl;
        outputFile << current_obj_scale.x << " " << current_obj_scale.y << " " << current_obj_scale.z << std::endl;
    }

    outputFile.close(); // 파일 스트림 닫기
}
...
bool load_asset(const std::string& filename)
{
  ...
  if (curr_scene != NULL)
  {
    for (unsigned int i = 0; i < curr_scene->mNumMeshes; ++i) 
    {
      ...
      // std::cout<<curr_scene->mMeshes[i]->mNumFaces<<std::endl;
...
void render_scene()
{
  ...
  for (std::size_t i = 0; i < g_objects.size(); ++i)
  {
    ...
    glm::mat4 mat_PVM = mat_proj * mat_view * g_objects[i].get_model_matrix();
    glUniformMatrix4fv(loc_u_PVM, 1, GL_FALSE, glm::value_ptr(mat_PVM));
    // glUniformMatrix4fv(loc_a_position, 1, GL_FALS)
